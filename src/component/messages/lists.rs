...
use gtk::prelude::*;
use relm4::{
  typed_view::{column::{LabelColumn, RelmColumn}, OrdFn},
  *,
};
use chrono::prelude::*;
use crate::{backend::repository::{KrustHeader, KrustMessage}, DATE_TIME_FORMAT};
...
impl LabelColumn for MessageTimestampColumn {
  ...
  fn format_cell_value(value: &Self::Value) -> String {
    format!("{}", Utc.timestamp_millis_opt(*value).unwrap().with_timezone(&America::Sao_Paulo).format(DATE_TIME_FORMAT))
  }
}
